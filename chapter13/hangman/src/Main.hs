...
gameOver (Puzzle wordToGuess filledInSoFar guessed) = do
  print numIncorrectGuessed
  if numIncorrectGuessed > 7 then
    do putStrLn "You lose!"
       putStrLn $ "The word was: " ++ wordToGuess
       exitSuccess
  else return ()
  where numIncorrectGuessed = length (nub guessed) - length (nub (intersect (catMaybes filledInSoFar) guessed))
